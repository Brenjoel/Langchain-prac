...
def set_env_variables(var_name , var_value):
    """
    Creates and sets an environment variable
    Args:
        var_name : name of the env variable
        var_value: Value for the env variable
    """
    if var_name!="":
        os.environ[var_name] = var_value

def HelloWorldLangChain():
    '''
    Basic function that implements langchain
    '''
    ...
    chain = summary_prompt_template | llm # lang chain expressiion language (LCEL) # this is a pipe connecting two components summary template and llm # the output of the summary template is given as input to the llm and the out put of llm is given as output #  this chain creates a runnable object, which mwans we can invoke the chain variable     
    response = chain.invoke(input = {"information": data , "point" : "heading for the information"})
    print("Response:\n",response.content)
    print("\n Done")    
